...
void main()	{
	...
	if (bleedAmount > 0.0)	{
		texture2D(al_tex, loc + vec2(0.01, 0)).r;
		bleed += texture2D(al_tex, loc + bleedRange * vec2(0.02, 0)).r;
		bleed += texture2D(al_tex, loc + bleedRange * vec2(0.01, 0.01)).r;
		bleed += texture2D(al_tex, loc + bleedRange * vec2(-0.02, 0.02)).r;
		bleed += texture2D(al_tex, loc + bleedRange * vec2(0.0, -0.03)).r;
		bleed /= 6.0;
		bleed *= bleedAmount;
	}
